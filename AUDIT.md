...
Seven new SA commands added for services root access:

| Command | Privilege | Risk | Notes |
|---------|-----------|------|-------|
| SAJOIN | PRIV_SERVICE | Medium | Bypasses all join restrictions |
| SAPART | PRIV_SERVICE | Low | Force part only |
| SANICK | PRIV_SERVICE | Medium | Nick collision check in place |
| SAMODE | PRIV_SERVICE | High | Can set any user/channel mode |
| SAQUIT | PRIV_SERVICE | Medium | Force disconnect |
| SATOPIC | PRIV_SERVICE | Low | Topic change only |
| SAWHOIS | PRIV_SERVICE | Low | Cosmetic WHOIS line |

All SA commands:
- Check `HasPriv(sptr, PRIV_SERVICE)` before execution
