 ...
 *
 * SA* (Server Admin) commands for Services Root access.
 * These are the oper-facing equivalents of the SVS* S2S protocol.
 * All require +k (channel service flag, set by services or oper block) — services root level access.
 *
...
/*
 * mo_sajoin - SAJOIN <nick> <#channel[,#channel2,...]>
 *
 * Forces a user to join one or more channels, bypassing all restrictions.
 * Requires +k (set by services like X3/Anope, or service=yes in oper block).
 */
...
/*
 * mo_sapart - SAPART <nick> <#channel[,#channel2]> [:<reason>]
 *
...
int mo_sapart(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  if (!MyUser(acptr)) {
    if (comment)
      sendcmdto_serv_butone(&me, CMD_SVSPART, cptr, "%C %s :%s",
                             acptr, parv[2], comment);
    else
 ...
 * For channels, delegates to OPMODE. For users, uses SVSMODE path.
 * Requires +k (set by services like X3/Anope, or service=yes in oper block).
 */
int mo_samode(struct Client* cptr, struct Client* sptr, int parc, char* parv[])
{
  ...
  /* Channel mode — delegate to OPMODE which already handles override */
 ...
 *
 * Forces a topic change on a channel, bypassing +t and other restrictions.
 * Requires +k (set by services like X3/Anope, or service=yes in oper block).
 */
