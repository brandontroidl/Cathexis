...
### SA* Commands — Moved to Services Root Only

SA* commands (SAJOIN, SAPART, SANICK, SAMODE, SAQUIT, SATOPIC,
SAWHOIS) are now services-protocol commands, not direct oper
commands. They can only be executed by services (X3, Atheme, etc.)
on behalf of a services root user.

This matches the deployment model where:
- Opers use /KILL, /GLINE, /OPMODE for direct network management
- SA* commands are issued through services by privileged users
- SVS* commands remain as the S2S transport layer

SAJOIN/SAPART/SANICK/SAMODE/SAQUIT/SAWHOIS all map to their SVS*
equivalents. SATOPIC has its own ms_satopic handler in m_sa.c.

### SA* Commands — Services Root Access (updated)

SA commands now require PRIV_SERVICE (+k, `service = yes` in oper block)
instead of PRIV_NETADMIN. This matches the services root access model:
only operators with service-level trust can execute forced actions.

The oper hierarchy for SA* access:
  service = yes  →  +k mode set  →  SA* commands enabled

This integrates with X3 services: when X3 sets +k on a user, they
gain SA* command access through the ircd.
